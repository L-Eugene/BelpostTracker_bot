...
namespace :belpost do
  namespace :db do
    task :initdb do
      ActiveRecord::Base.establish_connection Belpost::Config.instance.options['database']

      ActiveRecord::Tasks::DatabaseTasks.db_dir = '.'
      ActiveRecord::Tasks::DatabaseTasks.migrations_paths = Belpost::Config.instance.options['basedir'].to_s
    ...
    task version: :initdb do
      db_name = ActiveRecord::Base.connection.try(:current_database) ||
                Belpost::Config.instance.options['database'][:database]
